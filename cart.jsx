...
let Products = () => {
   ...
   let deleteItemFromCart = (element, index) => {
      ...
      setCart(updateCart);
      console.log(`Delete From Cart ${JSON.stringify(element.name)}`);
   }
   ...
   let list = items.map((element, index, array) => {
      return (
         <li key={index}>
            ...
            <input name={element.name} type="submit" onClick={addItemToCart}/>
         </li>
      );
   });
   ...
   let cartList = cart.map((element, index, array) => {
      return (
         <Accordion.Item key={index + 1}>
            ...
            <Accordion.Body onClick={() => deleteItemFromCart(element, index)}><Button>Delete From Cart (X)</Button></Accordion.Body>
         </Accordion.Item>
      );
   })
